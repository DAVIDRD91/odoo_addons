...
class CashFlowWizard(models.TransientModel):
    ...
    start_date = fields.Date(string="Data Inicial",default="")

    end_date = fields.Date(
        string="Data Final", required=True,
        default=fields.date.today()+datetime.timedelta(1*365/12))
    start_amount = fields.Float(string="Valor Inicial", digits='Account')
    ...
    def button_calculate(self):
        ...
        if not self.print_report:
            return self.env.ref('account_cash_flow.account_cash_flow_html_report').report_action(cashflow_id)
